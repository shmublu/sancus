...
def createUpgradeState(initial,target):
    ...
    for newChild in newList:
        newChild.node.listOfChildren=findChildren(newChild.node,initial,target)
    ...
    if(graph.listOfChildren==None):
        ...
        for child in graph.listOfChildren:
            ...
            if(lookChildren!=None):
                return lookChildren
        return None
#parses the JSON config file into a Node datat struct we cna analyze
...
jText='{"contracts": [{"name": "A","children": [{"name": "B","connect_sec": 1},{"name": "C","connect_sec": 0}]},{"name": "B", "children": [{"name":"D", "connect_sec": 0}]},{ "name": "C"}, {"name":"D"}]}'
iText='{"contracts": [{"name": "A","children": [{"name": "B","connect_sec": 0},{"name": "C","connect_sec": 0}]},{"name": "B", "children": [{"name":"D", "connect_sec": 0}]},{ "name": "C"}, {"name":"D"}]}'
A1=parseJSON(init)
A2=parseJSON(target)
